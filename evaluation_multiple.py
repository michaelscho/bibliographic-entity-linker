...
# LLM
# Currently based on gemini as proof of concept
    ...
    def process(rows, strict_mode):
        ...
        for db_author, db_title, db_year, db_place, vd_id in rows:
            ...
            if score >= MIN_ACCEPT_SCORE:
                out.append({"vd_id": vd_id, "vd_id_norm": normalize_db_vd_id(vd_id), "score": min(score, 100), "source": "RUN_1", "year": db_year, "title": db_title})
        return out
...
def execute_run_3(conn, broad_q, norm_query_fuzz, raw_query):
    ...
    for db_author, db_title, db_year, db_place, vd_id in rows:
        ...
        if score >= MIN_ACCEPT_SCORE:
            out.append({"vd_id": vd_id, "vd_id_norm": normalize_db_vd_id(vd_id), "score": min(score, 100), "source": "RUN_3", "year": db_year, "title": db_title})
...
def execute_run_4(conn, norm_query_fts, norm_query_fuzz):
    ...
    for db_author, db_title, db_year, db_place, vd_id in rows:
        ...
        if score >= 65:
            out.append({"vd_id": vd_id, "vd_id_norm": normalize_db_vd_id(vd_id), "score": min(score, 100), "source": "RUN_4", "year": db_year, "title": db_title})
...
"""
    ...
    2. Expand abbreviated terms in the query and candidate records to find matches (e.g., 'Hist.' = 'Historie' or 'Evangel.' = 'Evangelische').
    3. PREFERENCE RULE: If two records are similar, ALWAYS prefer the one with a 'vd18' prefix in the RECORD_ID over 'vd17'.
    4. Be flexible with OCR errors (e.g., 'Bot' = 'Botschaft') and partial title matches.
    5. If the query starts with 'Eine neue...' and the candidate says 'Besondere...' (or similar variations), accept it if the core nouns match.
    6. If the query has no year, find the best text match regardless of year, but still prefer 'vd18' IDs.
    7. Return the RECORD_ID of the best match. If nothing matches, return "NONE".
    """
...
def get_search_results(query: str, db_path: Path, query_place: str, query_author: str):
    ...
    try:
        ...
        llm_winner_id = "-"
        if final_list:
            top_cands = final_list[:50] # Wide net
    ...
    if not TEST_SET_PATH.exists():
        ...
        for entry in data:
            ...
            for b, gt in zip(bibl_items, gt_items):
                ...
                q_year = extract_query_year(q)
                
                raw_vd18 = gt.get("VD18_ID", "")
                targets = []

                if isinstance(raw_vd18, list):
                    for item in raw_vd18:
                        t = extract_vd18_id_from_gt(str(item))
                        if t: targets.append(t)
                    target_display = ", ".join(str(x).replace(" ", "") for x in raw_vd18)[:15]
                else:
                    t = extract_vd18_id_from_gt(str(raw_vd18))
                    if t: targets.append(t)
                    target_display = str(raw_vd18).replace(" ", "")[:15]
                ...
                hit = any(r["vd_id_norm"] in targets for r in final_res[:5])
                ...
                if not q_year and len(final_res) > 0:
                    print(f"   ⚠️  AMBIGUOUS (No Date): Displaying top 5 candidates:")
                    for idx, cand in enumerate(final_res[:25]):
                        is_winner = "(*)" if cand['vd_id_norm'] in targets else ""
                        print(f"       {idx+1}. {cand['vd_id']:<15} | Year: {cand.get('year', '?'):<4} | Score: {cand['score']:<3} | {cand.get('title', '')[:60]} {is_winner}")
                    print("   " + "-" * 50)
